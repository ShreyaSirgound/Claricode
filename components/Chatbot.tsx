...
export const Chatbot: React.FC<ChatbotProps> = ({ lesson, onUpdateLesson }) => {
  ...
  return (
    <div className="chatbot-wrapper flex flex-col h-full bg-gray-100 text-gray-900 dark:bg-[#212121] dark:text-gray-200 backdrop-blur-lg border-l border-gray-300 dark:border-[#4A4A4A] relative" style={{ width: `${width}px` }}>
      {/* Resize Handle */}
      <div
        className="absolute left-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-500 transition-colors group"
        onMouseDown={() => setIsResizing(true)}
      >
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-16 bg-gray-400 dark:bg-gray-600 group-hover:bg-blue-500 rounded-full transition-colors" />
      </div>
